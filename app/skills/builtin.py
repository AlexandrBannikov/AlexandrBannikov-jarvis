...
def build_skill_registry(tool_registry: ToolRegistry, config: Config, *, memory_manager=None, reminder_service=None, reminder_scheduler=None, ssh_dependencies=None) -> SkillRegistry:
    ...
    entries: list[tuple[SkillMetadata, Callable[[], tuple[HealthStatus, str]] | None]] = [
        (SkillMetadata("core", "Core", "1.0", "Core local diagnostic capabilities", "core", True, True, "builtin", tuple(name for name in names if name not in memory_names + reminder_names + ssh_names), capabilities=("local_diagnostics",), permissions=("read_local_status",)), lambda: _check_core(tool_registry)),
