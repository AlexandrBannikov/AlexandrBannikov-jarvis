...
_PATTERNS = (
    ...
    re.compile(r"(?i)\b(?:api[_ -]?key|secret|token|password|passwd|cookie|"
               r"authorization|telegram_bot_token|openai_api_key)\s*[=:]\s*[^\s,;]+"),
    re.compile(r"-----BEGIN (?:RSA |OPENSSH |EC )?PRIVATE KEY-----"),
    re.compile(r"\b\d{8,12}:[A-Za-z0-9_-]{20,}\b"),
    re.compile(r"(?i)\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\."),
    re.compile(r"(?i)(?:^|[/\\])\.env(?:$|[/\\\s])"),
    # Existing policy also treats private infrastructure coordinates as sensitive.
