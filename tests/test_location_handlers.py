...
def test_location_message_waits_for_consent(to_thread):
    ...
    to_thread.assert_awaited_once_with(service.resolve,1,2);service.save.assert_not_called();assert c.application.bot_data["pending_locations"][123] is candidate
@patch("app.handlers.asyncio.to_thread",new_callable=AsyncMock)
def test_save_uses_callback_owner(to_thread):
    service=Mock();c=ctx(service);candidate=LocationCandidate(1,2,None,None,"UTC");c.application.bot_data["pending_locations"]={123:candidate,999:object()}
    query=SimpleNamespace(data="location:save",answer=AsyncMock(),edit_message_text=AsyncMock());asyncio.run(location_callback(SimpleNamespace(callback_query=query,effective_user=SimpleNamespace(id=123)),c))
...
def test_discard_never_saves():
    service=Mock();c=ctx(service);c.application.bot_data["pending_locations"][123]=object();query=SimpleNamespace(data="location:discard",answer=AsyncMock(),edit_message_text=AsyncMock())
